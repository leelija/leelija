function empStatus()
{
	...
	//alert(txtAdhar);
	var url= "employee-status.inc.php?txtAdhar=" + escape(txtAdhar);
	request.open('GET',url,true);
	
	//set up a function to the server when its done
	request.onreadystatechange = getEmpStatus;
	
	//writing response while verifying
	document.getElementById('empStatusRpt').innerHTML=
...
function getEmpStatus()
{
	...
	if(request.readyState == 4)
	{
		...
		if(request.status == 200)
		{
			...
			//alert(xmlResponse);
			document.getElementById("empStatusRpt").innerHTML = xmlResponse;
		}

...
{	
	...
	$txtIGSTRate			= $_POST['txtIGSTRate'];
	
	//$txtSIGST	 		= $_POST['txtSIGST'];
	//registering the post session variables
	$sess_arr	= array('txtSupName','txtBillNo', 'txtBillingName', 'txtBalance', 'txtSGSTRate', 'txtCGSTRate',
	...
	else
	{
		...
		$igst	= $txtBalance * ($txtIGSTRate / 100);
		//add Supplier Bill Records
		$supplier->addSuppBillEnt($txtSupName,$txtBillNo,$txtBillingName, $txtBalance,$txtSGSTRate,$sgst, $txtCGSTRate,$cgst,
		$txtIGSTRate,$igst,$txtNetBalance,$txtProdDesc,$userData[2]);
		
		$suppDtls 		= $supplier->showSupplier($txtSupName);
		$sBalance 		= $suppDtls[6] + $txtNetBalance;
		//Edit Supplier Record
		$supplier->UpdateSupplier($txtSupName,$suppDtls[1],$suppDtls[2], $suppDtls[3],$suppDtls[4],$suppDtls[5], 
		$sBalance,$userData[2]);
		//Account details
		//$bAccDtls 		= $company->showCompAcc($txtBillingName);
		$cAccDtls 		= $company->showCompDtls($txtBillingName);
		$tsgst			= $cAccDtls[5] + $sgst;
		$tcgst			= $cAccDtls[6] + $cgst;
		$tigst			= $cAccDtls[7] + $igst;
		
		//update company details
		$company->updateCompDtls($cAccDtls[0],$cAccDtls[1],$cAccDtls[2], $cAccDtls[17],$cAccDtls[3],$cAccDtls[4],$tsgst, $tcgst, 
		$tigst, $cAccDtls[8],$cAccDtls[9], $cAccDtls[10],$userData[2]);
		
		$suppAccDtls 		= $supplier->showSuppAccCompWise($txtSupName,$txtBillingName);
		if($txtSupName == $suppAccDtls[1] && $txtBillingName == $suppAccDtls[2])
			{
				$ctBalance 	= $suppAccDtls[3] + $txtNetBalance;
				$supplier->UpdateSuppAccBlnc($txtSupName,$txtBillingName, $ctBalance);
			}
		else{
				$supplier->addSupplierAcc($txtSupName,$txtBillingName, $txtNetBalance);
			}
		...
		$uMesg->showSuccessT('success', 0, '', 'supplier-bill-entry.php', "Bill Record has been successfully added", 'SUCCESS');
	}
...
if(isset($_POST['btnCancel']))
{
	...
	//forward
	header("Location: supplier-bill-entry.php");
}
?>
...
<title><?php echo COMPANY_S; ?> - New Bill Entry for Supplier</title>
        ...
        <div class="inner-container">
            ...
            <div id="admin-body">
                ...
                <div class="webform-area">
                    ...
                    <?php 
					if(isset($_GET['action']) && ($_GET['action'] == 'add_suppBill')) 
					{	
					?>
						<br>
                        ...
                        <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data">
                            <div class="cl"></div>
							<div >
								<label>Supplier Name<span class="orangeLetter">*</span></label>							
								<select name="txtSupName" type="text" id="txtSupName" class="text_box_large">
								<?php
								$supDetails         = $supplier->getSupplierData();
								foreach ($supDetails as $row){//Array or records stored in $row
								?>
									<option value="<?php echo $row['sid'];?>"><?php echo $row['scompany'];?></option> 
								<?php
							...
							<label>IGST(%)<span class="orangeLetter"></span></label>
                            <input name="txtIGSTRate" type="text" class="text_box_large" id="txtIGSTRate" 
					        value="<?php $utility->printSess2('txtIGSTRate',''); ?>" size="25" />
                            <div class="cl"></div>
    ...
    $("#txtIGSTRate").keyup(function() {
       // var grandTotal = 0;
        $("input[name='txtBalance']").each(function (index) {
			...
			var txtIGSTRate = $("input[name='txtIGSTRate']").eq(index).val();
			var sgst		= parseFloat(txtBalance) * parseFloat(txtSGSTRate) / 100;
			var cgst		= parseFloat(txtBalance) * parseFloat(txtCGSTRate) / 100;
			var igst		= parseFloat(txtBalance) * parseFloat(txtIGSTRate) / 100;
            var txtNetBalance 		= parseFloat(txtBalance) + parseFloat(sgst) + parseFloat(cgst)+ parseFloat(igst);
			//alert(txtNetBalance);

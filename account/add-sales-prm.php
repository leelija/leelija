...
{	
	...
	$sess_arr	= array('txtCustName','txtBillNo', 'txtBillingName', 'txtBalance', 'txtSGSTRate', 'txtCGSTRate',
	...
	else
	{
		...
		$buyer->addSalesPrmBill($txtCustName,$txtBillNo,$txtBillingName, $txtBalance,$txtSGSTRate,$sgst, $txtCGSTRate,$cgst,
		$txtIGSTRate,$igst,$txtNetBalance,$txtProdDesc,$userData[2]);
		
		$custDtls 		= $buyer->showBuyer($txtCustName);
		$cBalance 		= $custDtls[12] - $txtNetBalance;
		//Edit Customer Record
		$buyer->editBuyer($txtCustName,$custDtls[1], $custDtls[14],$custDtls[2], $custDtls[4], $custDtls[15], $cBalance, $userData[2]);
		//update customer acc details
		$custAccDtls 		= $buyer->showBuyerAccCompWise($txtCustName,$txtBillingName);
		$custAccBalance		= $custAccDtls[3] - $txtNetBalance;
		$buyer->UpdateBuyerAccBlnc($txtCustName,$txtBillingName, $custAccBalance);
		
		//Account details
		$cAccDtls 		= $company->showCompDtls($txtBillingName);
		$otsgst			= $cAccDtls[8] - $sgst;
		$otcgst			= $cAccDtls[9] - $cgst;
		$otigst			= $cAccDtls[10] - $igst;
		...
		$uMesg->showSuccessT('success', 0, '', 'sales-prm.php', "Sales PRM Bill Record has been successfully added", 'SUCCESS');
	}
...
<title><?php echo COMPANY_S; ?> - New Bill Entry for Sales PRM</title>
					...
					?>
                        ...
                        <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data">
							...
							<div >
								...
								<select name="txtCustName" type="text" id="txtCustName" class="text_box_large">
								<?php
								$custDetails         = $buyer->buyerDtl();
								foreach ($custDetails as $row){//Array or records stored in $row
								?>
									<option value="<?php echo $row['party_id'];?>"><?php echo $row['company'];?></option> 
								<?php
							...
							<div class="cl"></div>
							<div >
								<label>Billing Issue Form<span class="orangeLetter">*</span></label>							

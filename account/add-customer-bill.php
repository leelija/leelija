...
{	
	...
	else
	{
		...
		//add Customer Bill Records
		$party->addCustBillEnt($txtCustName,$txtBillNo,$txtBillingName, $txtBalance,$txtSGSTRate,$sgst, $txtCGSTRate,$cgst,
		$txtIGSTRate,$igst,$txtNetBalance,$txtProdDesc,$userData[2]);
		
		$custDtls 		= $party->showParty($txtCustName);
		$cBalance 		= $custDtls[12] + $txtNetBalance;
		//Edit Customer Record
		$party->editParty($txtCustName,$custDtls[1], $custDtls[14],$custDtls[2], $custDtls[4], $custDtls[15], $cBalance, $userData[2]);
		
		
		//Account details
		$bAccDtls 		= $company->showCompAcc($txtBillingName);
		$otsgst			= $bAccDtls[8] + $sgst;
		$otcgst			= $bAccDtls[9] + $cgst;
		$otigst			= $bAccDtls[10] + $igst;
		...
		$uMesg->showSuccessT('success', 0, '', 'customer-bill.php', "Bill Record has been successfully added", 'SUCCESS');
	}
...
if(isset($_POST['btnCancel']))
{
	...
	//forward
	header("Location: customer-bill.php");
}
?>
...
<title><?php echo COMPANY_S; ?> - New Bill Entry for Customer</title>
					...
					?>
                        ...
                        <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data">
							...
							<div >
								...
								<?php
								$custDetails         = $party->partyDtl();
								foreach ($custDetails as $row){//Array or records stored in $row
								?>
									<option value="<?php echo $row['party_id'];?>"><?php echo $row['party_name'];?>(<?php echo $row['party_id'];?>)</option> 
								<?php

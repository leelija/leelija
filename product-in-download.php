...
<title><?php echo COMPANY_S; ?> - Product In Details</title>
                ...
                <div id="data-column">
                	...
                	<table id="testTable">
                       ...
                       $sid = array_slice($sid, $start, $limit);     
                        foreach($sid as $x)
							...
							$stitchDetails  	= $statusCat->disPlTableDtls($statusCatDetail[17],'final_stich');
							$ordDtlDtls			= $order->getOrderQuantitycount($statusCatDetail[2]);
							foreach($ordDtlDtls as $eachRecordc)
								{
									$ordQty 	= $eachRecordc['SUM(quantity)']; 
								}					
							// Due quantity
							$prodDueQty 		= $ordQty - $statusCatDetail[15];
					...
					<?php if($prodDueQty !=0){ ?>
                      <tr align="left"<?php $utility->printRowColor($bgColor);?>>
					   ...
					   <td><?php $order->TotalQuantitySum($statusCatDetail[2]);	?></td>
					   <td><?php echo $mstName; ?></td>
					   ...
					   <td><?php echo $statusCatDetail[13];?></td>
						...
						<td>
							<a href="#" onClick="MM_openBrWindow('packing_comp.php?action=status_details&bid=<?php echo $statusCatDetail[17]; ?>','AdminDelete','scrollbars=yes,width=500,height=400')">
						...
						<td> <?php $stockInData 	= $productStock->stockCompInData($statusCatDetail[17]);
							foreach( $stockInData as $eachrecord ){
								echo $eachrecord ['colour']; echo "-";echo $eachrecord ['product_in'];echo "<br>";
								}
							?>

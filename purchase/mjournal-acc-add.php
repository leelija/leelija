...
{	
	...
	$sess_arr	= array('txtAccName','txtDescription');
...
if($txtAccName == '')
	{
		echo "Account Name field empty";
	}
	
	else
	{
		//add the journal account
		$journal->addMJournalAcc($txtAccName,$txtDescription,'',$userData[2]);	
		//deleting the sessions
		$utility->delSessArr($sess_arr);
		
		//forward the web page
		$uMesg->showSuccessT('success', 0, '', $_SERVER['PHP_SELF'],'Journal Account has been successfully added', 'SUCCESS');
	}
}
...
<title><?php echo COMPANY_S; ?> -  - Add Journal Account</title>
...
<h3><a name="editStock"></a></h3>
          ...
          <form action="<?php $_SERVER['PHP_SELF']?>?action=journalAcc" method="post" enctype="multipart/form-data">
			...
			<h2>Journal Account Add</h2>
                <span>Please  <span class="required"></span>fill the following field.</span>
                <div class="cl"></div>
							
				<label>Account Name<span class="orangeLetter">*</span></label>
                <input name="txtAccName" type="text" class="text_box_large" id="txtAccName" size="25" />
                <div class="cl"></div>
							
				<label>Description<span class="orangeLetter">*</span></label>
                <input name="txtDescription" type="text" class="text_box_large" id="txtDescription" size="25" />

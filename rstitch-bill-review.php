...
<!-- /TinyMCE -->
    ...
    <div class="webform-area">		
        ...
        {
			$stitchDetails  		= $statCat->disPlTableDtls($bid,'final_stich');
			$billDtl 				= $bill->allRFRIBillTable($bid,'rfri_fs_bbook');
			//factory details
			...
			<div class="print-cont" style="width:80%; margin:0 auto;">
                ...
                <div id="data-column" style="position:relative; bottom: 9px; font-size:12px;">
						...
						<tbody>
						   <?php 
							...
							foreach($stitchDetails as $eachRecord)
								...
								$oid				= $eachRecord['order_id'];
								$_SESSION['remarks'] = $eachRecord['remarks'];
								$custDtls 			= $customer->getCustomerData($eachRecord['employeeId']);
								$ordDtls 			= $orders->showOrders($eachRecord['order_id']);
								$countSamedQuty 	= $orders->getSameDes($eachRecord['order_id']);
					...
					<div class="tBillTitle" style="background:#ddd; height: 30px;">
						<p class="ltotal" style="width:50%; float:left;">Total(<?php echo $tsameQty;?> Psc)</p>
					...
					<?php
						...
						foreach( $orderColorDtl as $eachrecord ){
					?>
					 <b style="font-size:16px;"><?php echo $eachrecord ['colour']; ?>-<?php echo $eachrecord ['quantity']; ?></b><br>
					<?php
					}
					?>
					<!--<h2>Particulars</h2>-->
					...
					?>
					<b>Remarks:<?php echo $_SESSION['remarks'];?><br>
					ORDER NOTE: <?php echo $ordNote;?>
					</b>
					<p>Order Date: <?php echo date_format($ordDate,"d/m/Y"); ?></p>
					<p>Target Date: <?php echo date_format($trdDate,"d/m/Y"); ?></p>
					<p style="font-size:18px;">Order By:<?php echo $ordBy;?></p>
                    <div class="first-column">
                  </div>
                  
                </div>
				<div style="width: 100%;">
					<div style="width: 50%; float: left;"> <p style="width: 48%; float: left;">Iron Man:</p>
					<p style="width: 50%; float: right;">Amount:</p> </div>
					<div style="width: 48%;float:right; ">
						<p style="width: 60%; float: left;">InterLock Man:</p>
						<p style="width: 38%; float: right;">Amount:</p>
					</div>
				</div>
				<div style="width: 100%;">
					...
					<div id="dvData">   	
						...
						<table width="100%" class="single-column" cellpadding="0" cellspacing="0">
							<thead width="100%">
								...
								<th style="text-align:center;border: solid 1px #000;">Karigor Name</th>
								...
								<th style="text-align:center;border: solid 1px #000;">Colour</th>
		...
		<div id="PrintCuForm" ><!-- start Print -->
			<div class="print-cont" style="width:80%; margin:0 auto;">
				<h2 style="text-align: center;"><a name="addUser">Job Work Chalan For Cutting</a></h2>
				<div style="width: 100%;">
					<p style="width: 50%; float: left;">No.: <b>S<?php echo $bid;?></b></p> 
					<p style="width: 48%;float:right; text-align: right;">Date: <?php echo date_format($bDate,"d/m/Y"); ?></p>
				</div>
				...
				<p style="border-bottom: 1px dashed #999;">Messrs &nbsp;<?php //echo $bUserDtls[5]; ?>Farid &nbsp;<?php //echo $bUserDtls[6]; ?>Sk.</p>
                ...
                <div id="data-column" style="position:relative; bottom: 9px; font-size:12px;">
					...
					<div class="tBillTitle" style="background:#ddd; height: 30px;">
						<p class="ltotal" style="width:50%; float:left;">Total(<?php echo $tsameQty;?> Psc)</p>
						<p class="rtotal" style="width: 45%; float: right;position: relative;left: 205px;"><b>Rs.00</b></p>
					</div>
					<?php
						$orderColorDtl		= $orders->ordersDtlDisplay($oid);
						foreach( $orderColorDtl as $eachrecord ){
			...
			<?php if($tAmount !=0){?>
				<a onclick="PrintCuttDiv();" class="add-icon tooltip1" title="Print Bill Details">Print</a>
			<?php
			}else{}
			?>
	...
	<!--Used for print-->
	<script type="text/javascript">     
        function PrintCuttDiv() {    
           var PrintCuForm = document.getElementById('PrintCuForm');
           var popupWin = window.open('', '_blank', 'width=800,height=800');
           popupWin.document.open();
           popupWin.document.write('<html><body onload="window.print()">' + PrintCuForm.innerHTML + '</html>');
           popupWin.document.close();
                }
    </script>

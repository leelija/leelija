...
//echo $keyword;exit;
	if($_POST['type'] ==""){
		$link = '';
		$sid = $search_obj->getProductKeywordpack($keyword);
		//echo $sid;exit;
	}
	else{
		$link = '';
		$sid = $advSearch->getCommonSearch($keyword,$type,$userId,'packing_id','packing_table');
...
else
{
	...
	$sid = $statusCat->getAllPaking('packing_id','DESC',$userId);
}
/*	GET ALL PRODUCT*/
//print_r($pids);exit;
        ...
        <div class="inner-container">
            ...
            <div id="admin-body">
              ...
              <div class="padT30"><!-- --> </div> 
			   ...
			   <!-- Download option -->
                <div id="options-area">
					<a href="product-in-download.php">View & Download</a>
                </div>
                <!-- eof Download option -->
				...
				<!--		  <th width="7%" >Packing Id</th>
                       ...
                       $sid = array_slice($sid, $start, $limit);     
                        foreach($sid as $x)
							...
							// update factory
							$status->updatePipeLine($statusCatDetail[2],$ordDtls[14],'packing_table');
							
							$ordDtlsCnt 	= $order->getOrderQuantitycount($statusCatDetail[2]);
							foreach($ordDtlsCnt as $eachQty)
								{
									$OrdQty 	= $eachQty['SUM(quantity)']; 
								}
							if($statusCatDetail[15] == $OrdQty){
								$statusCat->UpdatePLineST($statusCatDetail[2], 'Complete','packing_table');
								$status->updateStableSt($statusCatDetail[2],'Complete','packing');
							}else{
								$statusCat->UpdatePLineST($statusCatDetail[2], 'Incomplete','packing_table');
								$status->updateStableSt($statusCatDetail[2],'Incomplete','packing');
							}
                    ...
                    ?>
                      <tr align="left"<?php $utility->printRowColor($bgColor);?>>
					   ...
					   <td align="center">
						...
						<td align="center">
							<p style='color:blue' onClick="MM_openBrWindow('packing_comp.php?action=status_details&bid=<?php echo $statusCatDetail[17]; ?>','AdminDelete','scrollbars=yes,width=500,height=400')">
					...
					*/   ?>
					   <td align="center">
							<?php 
								if($statusCatDetail[16] == 'Complete'){
									echo $statusCatDetail[16]; 
									//update order submit date
									$order->editOrderSubmitmod($statusCatDetail[2],$statusCatDetail[13]);
								} else
									{
										echo $statusCatDetail[16];										
									}
					...
					//	  }
                        ...
                        <!-- [ 
                          <a href="#" onClick="MM_openBrWindow('packing_update.php?action=update_packing&dhmckfip=<?php echo $statusCatDetail[0]; ?>','AdminDelete','scrollbars=yes,width=700,height=600')">
                           Packing					  </a> ]
						-->   
						  <!-- [ 
							<a href="#" 
							  onClick="MM_openBrWindow('product_in.php?action=edit_sid&sid=<?php echo $statusCatDetail[0]; ?>','stockEdit','scrollbars=yes,width=750,height=600')">
							  Add To Stock					  </a> ]-->
							 [ 
							<a href="#" 
							  onClick="MM_openBrWindow('stock-prod-in.php?action=ProdIn&sid=<?php echo $statusCatDetail[0]; ?>','stockEdit','scrollbars=yes,width=750,height=600')">
							  Add To Stock				  </a> ]  

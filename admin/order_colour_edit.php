...
$oid_dtl_id			= $utility->returnGetVar('oid_dtl_id','0');
...
{	
	...
	{
		//edit orders
		 $orders->editOrdersDtl($oid_dtl_id,$txtQuantity, $txtColour);
...
<!-- /TinyMCE -->
    ...
    <div class="webform-area">		
		<?php 
        //form
        if( (isset($_GET['action'])) && ($_GET['action'] == 'edit_ord_colour') )
        {
            $orderColorDtl		= $orders->getOrdersDetails($oid_dtl_id); 
            //$ordDetail = $orders->showOrders($oid);
        ?>
		
<h3><a name="editProd">Edit Orders colours</a></h3>
    
          <form action="<?php $_SERVER['PHP_SELF']?>?action=edit_ord_colour&oid_dtl_id=<?php echo $oid_dtl_id; ?>" method="post" enctype="multipart/form-data">
         
		 
		 <h2><a name="addUser">Edit Orders Colours</a></h2>

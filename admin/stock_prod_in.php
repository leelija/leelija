...
if($txtDesignNo=='')
	...
	else
	{
		...
		for($i=0; $i < $selNum; $i++)
		{
			...
			$stockDetails = $stock->showStock($_SESSION['sid']);
			//admin detail
			...
			//
			$stockDtl		= $stock->DisStockCD($txtColour,$txtDesignNo);
	
			//edit stock
			$stock->editStock($stockDetails[0],$txtDesignNo, $txtStock, $totalProdIn, $stockDetails[4], $stockDetails[12], $stockDetails[5],
			$stockDetails[6], $stockDetails[7],$userData[10]);	
		
			// add product in
			$stock->addProductIn($txtDesignNo,$txtBillNo, $txtProductIn, $txtColour, $txtProdDesc, '', $userData[10]);
			//echo $txtStock;exit;
			if($txtStock == 0 || count($stockDtl) == 0){
		
				$stock->addstockDetails($stockDetails[0], $txtDesignNo, $txtColour, $txtProductIn,$txtProductIn,'','', '', $userData[10]);

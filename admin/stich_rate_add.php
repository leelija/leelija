...
{	
	...
	//field validation
	$duplicateId	= $error->duplicateUser($txtDesign, 'design_no', 'stich_rate');
	if($txtDesign=='')
	{
		$error->showErrorTA($action, $id, $id_var, $url, "Design No Is Empty", $typeM, $anchor);
	}
	elseif( ($txtDesign != '') && (preg_match("/^ER/i",$duplicateId)) )		
	{
		
		$error->showErrorTA($action, $id, $id_var, $url, "Design Rate already in use", $typeM, $anchor);
	}
	...
	//add the Orders
	    $rate->addStichRate($txtDesign,$txtParticular, $txtParticularRate);	
		...
		$uMesg->showSuccessT('success', 0, '', 'stich_rate.php', MSG001, 'SUCCESS');
	}
...
if(isset($_POST['btnCancel']))
{
	...
	//forward
	header("Location: stich_rate.php");
}
?>

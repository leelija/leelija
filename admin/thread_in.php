...
$current_stock= $txtAmount+$threadDtl[2];
...
if($txtAmount == '')
	...
	{
		//add the fabric
	   $thread->addThreadIn($threadDtl[1],$txtAmount,$customerDtl[16],$txtCompany,$txtFrom,$txtPrice,$totalAmount,$txtProdDesc);	
	   $thread->editThread($tid,$threadDtl[1],$current_stock,$threadDtl[5],$threadDtl[4]);
		...
		$uMesg->showSuccessT('success', 0, '', $_SERVER['PHP_SELF'], THROD001, 'SUCCESS');
	}
}
...
<!-- /TinyMCE -->
    ...
    <div class="webform-area">		
		<?php 
        //form
        if( (isset($_GET['action'])) && ($_GET['action'] == 'in_tid') )
        {
             
            $threadDetails = $thread->showThread($tid);
        ?>
		
<h3><a name="editStock">Thread In</a></h3>
    
          <form action="<?php $_SERVER['PHP_SELF']?>?action=in_tid&tid=<?php echo $tid; ?>" method="post" enctype="multipart/form-data">
         
		 
		 <h2><a name="addUser">Thread In</a></h2>

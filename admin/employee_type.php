...
if(isset($_POST['btnCreateType'])) 
{
	...
	{
		//add the type
		$type_id		= $customer->addCustomerType($parent_id, $txtCusType, $txtCode, $txtDesc);
...
/*				  if(isset($_GET['catId']))
                ...
                <div id="data-column">
                    ...
                    if($typeId > 0)
					{
						$typeIds		= $customer->getChildCustomerTypeId($typeId);
					}
					else
					{
						$typeIds		= $customer->getAllParentCustomerTypeId();
					}
                    $allTypes			= $customer->getAllCustomerTypeId();
                    ...
                    <thead>
                      <th width="6%">ID</th>
                      <th width="23%">Customer Type </th>
                      <th width="22%">Subcategory(ies)</th>
                      <th width="15%">No. of User</th>
                    ...
                    <?php 
                        ...
                        {
                            $typeDetail 	= $customer->getCustomerTypeData($k);
							$subTypeIds		= $customer->getChildCustomerTypeId($k);
                            $bgColor 		= $utility->getRowColor($i);
                    ?>
                        <tr align="left" <?php $utility->printRowColor($bgColor);?>>
                          <td><?php echo $i; ?></td>
                          <td><?php echo $typeDetail[1]; ?></td>
                          <td>
                          <?php 
                          ...
                          <td><?php echo $dateUtil->printDate($typeDetail[4]); ?></td>
                          <td>
                          <?php 
                          /*if($k != 1)
                          {*/
					...
					<span>Please note that all the <span class="required">*</span> marked fileds are required</span>
					  ...
					  <form action="<?php $_SERVER['PHP_SELF']?>" method="post" enctype="multipart/form-data">
							...
							<select name="txtParentId" class=" textBoxA" id="txtParentId">
							  ...
							  {
								$customer->customerTypeDropDown(0,0,$_GET['txtParentId'],'ADD',0,'customer_type');
							  }
							  elseif(isset($_SESSION['txtParentId']))
							  {
								$customer->customerTypeDropDown(0,0,$_SESSION['txtParentId'],'ADD',0,'customer_type');
							  }
							  else
							  {
								$customer->customerTypeDropDown(0,0,0,'ADD',0,'customer_type');
							  }
							  ?>
							</select>
                            <div class="cl"></div>

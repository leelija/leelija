...
{	
	...
	//add the Sample Fabric bill
	  $bill_id =  $bill->addSampleBilling($txtDesigner, $txtDyerName,$txtPurpose, '', '',$OrdersDate,$TargetDate,$txtProdDesc,$userData[2]);	
		
		for($i=0; $i < $selNum; $i++)
			{
				$tAmount 	= $_POST['txtRate'][$i] * $_POST['txtQuantity'][$i];
				//add the bill details
				$bill->addSampleBillingDtls($bill_id,$_POST['txtFabricName'][$i],
				$_POST['txtQuantity'][$i],$_POST['txtRate'][$i],$tAmount);	
				
				
				$samBillDtls 	= $bill->allSampleBilling($bill_id);
				$TotalAmount 		= $samBillDtls[4] + $tAmount;
				$bill->updateSampleBilling($bill_id,$TotalAmount);
			}
		...
		$uMesg->showSuccessT('success', 0, '', 'sample_billing.php', "Bill has been successfully added", 'SUCCESS');
	}
...
if(isset($_POST['btnCancel']))
{
	...
	//forward
	header("Location: sample_billing.php");
}
?>
...
<title><?php echo COMPANY_S; ?> - Sample Billing Details</title>

...
$stockDtls		= $productStock->ProductstockAllDisplay();
...
{
	$link = '';
	$sid = $productStock->getAllProdStock('stock_id','DESC');
}
/*	GET ALL PRODUCT*/
//print_r($pids);exit;
            ...
            <div id="admin-body">
               ...
               <div id="admin-top">
				...
				<div class="header-title" style="height:40px;">
					<h2 style="float:left;">Total Current Stock:<?php  $productStock->CurrentProdstockSum();?></h2>
					<h2 style="float:right;">Total Search Design found: <?php echo count($sid);?></h2>
				</div>	
                ...
                <div id="data-column" style="position:relative; bottom: 90px;">
                	<table id="example" class="single-column" cellpadding="0" cellspacing="0" >
                       ...
                       <?php 
                        ...
                        foreach($stockDtls as $eachRecord)
                            {
							...
							$grDtls 				= $productStock->showTGrProd($eachRecord['stock_id']);
							foreach($grDtls as $eachGr)
								{
									$tGr		= $eachGr['SUM(ready_gd)'];
								}	
							
							$netsales 		= $eachRecord['sales'] - $tGr;
							//Edit net sales
							$productStock->editProdstockNsales($eachRecord['stock_id'], $netsales);
							//factory details
                    ...
                    ?>
                      <tr align="left"<?php $utility->printRowColor($bgColor);?>>
						...
						if(count($photoGallDtls[6]) != 0)
						{
							...
							{
							$design 		= $eachRecord['design_no'];
						?>	
								
								<img onClick="showImageModal('<?php echo $eachRecord['design_no'];?>');" src="../images/spgallery/large/<?php echo $photoGallDtls[6];?>" width="50" height="50" 
								data-toggle="modal" data-target="#ImageModal"  
								title="<?php echo $eachRecord['design_no']; ?>" >
					   ...
					   <td align="center"><?php echo $eachRecord['stock']; ?></td>
						<td align="center"><?php echo $eachRecord['product_in']; ?></td>
					   <td align="center"><?php echo $eachRecord['sales']; ?></td>
					   <td align="center"><?php echo $netsales; ?></td>
					   <td align="center"><?php echo $eachRecord['goods_return']; ?></td>
					   <td align="center"><?php echo $factoryDtls[1]; ?></td>
					   <td align="center"><?php echo date_format($date,"m/d/Y"); ?></td>
					  
					   	<td >
							<p href="#" 
						  onClick="MM_openBrWindow('stock_prod_in.php?action=add_prod&sid=<?php echo $eachRecord['stock_id']; ?>','stockEdit','scrollbars=yes,width=750,height=600')">
						  Product In					  </p> 
						    
							<p href="#" 
						  onClick="MM_openBrWindow('product_sale.php?action=sale_sid&sid=<?php echo $eachRecord['stock_id']; ?>','stockEdit','scrollbars=yes,width=750,height=600')">
						  Sales					  </p> 
						  
						<!--	<p href="#" 
						  onClick="MM_openBrWindow('goods_return.php?action=edit_sid&sid=<?php echo $eachRecord['stock_id']; ?>','stockEdit','scrollbars=yes,width=750,height=600')">
						  GR					  </p> 
						  -->
						  <p href="#" 
						  onClick="MM_openBrWindow('products_return.php?action=edit_sid&sid=<?php echo $eachRecord['stock_id']; ?>','stockEdit','scrollbars=yes,width=750,height=600')">
						  GR					  </p> 
						 
[ 
					    <a href="order_add.php?action=add_ord&sid= <?php echo $eachRecord['stock_id']; ?>" style="color:red;">
					  Order					  </a> ]
					   ...
					   <!--   [ 
						...
						?>		
							<p href="#" 
						    onClick="MM_openBrWindow('stock_edit.php?action=sale_sid&sid=<?php echo $eachRecord['stock_id']; ?>','stockEdit','scrollbars=yes,width=750,height=600')">
						    Edit					  
							</p> 
						<?php
							}
							?>
						<!--  [ 
						  <a href="#" onClick="MM_openBrWindow('stock_delete.php?action=del_sid&sid=<?php echo $eachRecord['stock_id']; ?>','OrdDelete','scrollbars=yes,width=400,height=350')">
						  delete					  </a> ]<br>  -->
					</td>		
				     </tr>
                      <?php 
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="ImageModal"></h4>
				</div>
				  <div class="modal-body">
				  </div>
				  <div class="modal-footer">
					<div class="text-center">
					  <button type="button" class="btn btn-default" data-dismiss="modal" id="id_refresh"> Cancel </button>
					</div>
				  </div>      
			</div>
		</div>
...
<script>
function showImageModal(design_no){
	//alert(design_no);
	var design_no = design_no;
    var url = "zoom-image.php?design_no="+design_no;
    $(".modal-body").html('<iframe width="100%" height="1200" frameborder="0" scrolling="no" allowtransparency="true" src="'+url+'"></iframe>');

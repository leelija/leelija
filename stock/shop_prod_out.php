...
if($txtQuantity=='')
	...
	else
	{
		...
		//Add Product Out
		$rozStock->addShopProdOut($rzStockDtls[1],$txtQuantity,$userData[2]);
		//$rozStock->addShopProdOut($design_no,$outqty,$added_by);
...
<!-- /TinyMCE -->
    ...
    <div class="webform-area">		
		<?php 
        //form
        if( (isset($_GET['action'])) && ($_GET['action'] == 'OutProd') )
        {
            $rzStockDtls 	= $rozStock->showRozStock($sid);
        ?>
		 ...
		 <h3><a name="editStock"></a></h3>
          ...
          <form action="<?php $_SERVER['PHP_SELF']?>?action=OutProd&sid=<?php echo $sid; ?>" method="post" enctype="multipart/form-data">

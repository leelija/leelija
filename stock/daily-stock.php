...
$liveStock		= $dailyStock->LiveStockShow();
$sids 			= $dailyStock->getAllStock('stock_id', 'DESC');
...
if(isset($_POST['btnUpdate']))
{
	...
	foreach($sids as $k)
	{
		...
		if($newStock[$k] != '')
		{
			$utility->updateField($k, 'stock_id', (int)$newStock[$k], 'quantity', 'hmda_stock','YES','modified_on');
		}
	}
            ...
            <div id="admin-body">
                ...
                <div id="options-area">
					<div class="add-new-option">
						<a href="#" 
							  onClick="MM_openBrWindow('add-daily-stock.php?action=add_prod','stocAdd','scrollbars=yes,width=750,height=600')">
							  Add New Design					  
						</a>
					</div>		
                </div>
				<p style="font-size:20px;">Total Current Stock:<?php  $dailyStock->CurrentLiveStockSum();?></p><br>			
				...
				<form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post">
                	<table id="example" class="single-column" cellpadding="0" cellspacing="0" >
                       ...
                       <?php 
                        ...
                        foreach($sids as $x)
                            {
								$stockDetail 	= $dailyStock->showLiveStock($x);

...
{	
	...
	$txtProdDesc 	        = $_POST['txtProdDesc'];
		...
		$sess_arr	= array('txtReason','txtProdDesc');
...
if($txtReason == '')
	...
	else
	{
			  ...
			  $productOrder->addProdOrderCancel($orderDetail[0],$orderDetail[1],$txtReason,$txtProdDesc);
			  $productOrder->UpdateProdOrders($orderDetail[0],'cancel');
			  $productOrder->UpdateProdOrderDetails($orderDetail[0],'cancel');
		...
		$uMesg->showSuccessT('success', 0, '', $_SERVER['PHP_SELF'], "Order has been successfully Cancel", 'SUCCESS');
	}
}
...
<title><?php echo COMPANY_S; ?> -  - Deliver Rozelle Cancel</title>
...
<!-- /TinyMCE -->
    ...
    <div class="webform-area">		
		<?php 
        //form
        if( (isset($_GET['action'])) && ($_GET['action'] == 'cancel_ord') )
        {
...
<!--<h3><a name="editStock">Rozelle order Deliver</a></h3>-->
          ...
          <form action="<?php $_SERVER['PHP_SELF']?>?action=cancel_ord&oid=<?php echo $orderDetail[0]; ?>" method="post" enctype="multipart/form-data">
         
		 
		 <h2 style="padding-left:100px;"><a name="addUser">Rozelle order Cancel</a></h2>
					   ...
					   <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data">
						    ...
						    <div >
                                <label>Reason for cancellation<span class="orangeLetter">*</span></label>
                                <select name="txtReason" type="text" id="txtReason" class="text_box_large" >
                                ...
                                <option value="Ordered duplicate items by mistake" >Ordered duplicate items by mistake</option>
                                 <option value="Order wrong item" >Order wrong item</option>

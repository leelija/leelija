...
require_once("../classes/shop-stock.class.php"); 
...
$sStock			= $shopStock->ShopStockShow();
$sids 			= $shopStock->getAllShopStock('stock_id', 'DESC');
...
if(isset($_POST['btnUpdate']))
{
	...
	foreach($sids as $k)
	{
		...
		if($newStock[$k] != '')
		{
			$utility->updateField($k, 'stock_id', (int)$newStock[$k], 'quantity', 'shop_stock','YES','modified_on');
		}
	}
            ...
            <div id="admin-body">
                ...
                <div id="options-area">
					<div class="add-new-option">
						<a href="#" 
							  onClick="MM_openBrWindow('add-shop-stock.php?action=add_prod','stocAdd','scrollbars=yes,width=750,height=600')">
							  Add New Design					  
						</a>
					</div>	
					<a href="../file_download.inc.php?table=shop_stock">Download</a>
                </div>
				<p style="font-size:20px;">Total Current Stock:<?php  $shopStock->CurrentShopStockSum();?></p><br>			
				...
				<form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post">
                	<table id="example" class="single-column" cellpadding="0" cellspacing="0" >
                       ...
                       <?php 
                        ...
                        foreach($sids as $x)
                            {
								$stockDetail 	= $shopStock->showShopStock($x);

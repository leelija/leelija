...
if($sampleDtl[0] == '')
	...
	else
	{
				...
				//add the sample manual ari data
				$manual_arri_id 	= $sample->addSamManual($sampleDtl[0],$txtEmpName,$sampleDtl[3],$txtParticularName,$txtTime,
				$txtLabourCost,$labourTotalCost,$MaterialTotalCost,$txtOthersCost,
				$totalCost,$txtProdDesc,$userData[10],'sample_manual_aari');
		...
		for($i=0; $i < $selNum; $i++)
			{
				$sampleDtl				= $sample->showSampleDb($sid);
				$sampleManualAriDtl		= $sample->showSamManualari($manual_arri_id);
				
				$materialCost  		= $_POST['txtMatAmount'][$i] * $_POST['txtMaterialRate'][$i];	
				$MTotalCost 		= $sampleManualAriDtl[10] + $materialCost; // Manual Ari Total cost
				$material_cost 		= $sampleManualAriDtl[8] + $materialCost;
				
				$total_time 		= $sampleDtl[4];
				$TotalCost 			= $sampleDtl[5] + $materialCost;
				
				//add the sample_manualari_materials 
				$sample->addSamMaterials($manual_arri_id,$_POST['txtMaterialName'][$i],$_POST['txtMatAmount'][$i],
				$_POST['txtMUnit'][$i],$_POST['txtMaterialRate'][$i],$materialCost,'sample_manualari_materials','manual_arri_id');	
				
				//update sample_manual_aari
				$sample->UpdateSampleAllTable($manual_arri_id,$material_cost,$MTotalCost,'sample_manual_aari','manual_arri_id');
				// Update sample_db
...
<title><?php echo COMPANY_S; ?> -Add Sample Manual Aari  Material</title>
...
<h3><a name="editStock"></a></h3>
		 ...
		 <h2><a name="addUser">Add Sample Manual Aari Material</a></h2>

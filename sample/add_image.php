...
if(isset($_POST['btnAddOrd']))
{
	...
	if($_FILES['fileSubImg']['name'] == '')
		...
		$i		= 0;
			...
			//add orders image
		//	$ordImgId	= $orders->addOrdImage($sid, $txtSubImgTitle, $isDefault);
		...
		*/						   
			//rename the file
			$newName = $utility->getNewName4($_FILES['fileSubImg'], '',$sid);
			
			//resizing the image
			$uImg->imgCropResize($_FILES['fileSubImg'], '', $newName, 
								   'images/', 
								   800, 800, $sid,'image', 'sample_id', 'sample_db');					   
								   
								   
			//resizing the image
		/*	$uImg->imgCropResize($_FILES['fileSubImg'], '', $newName, 
								   '../images/products/order_img/', 
								   80, 80, $sid,'thumb_image', 'order_image_id', 'order_image');
		*/						   
		/*	$uImg->imgCropResizeArr($i, $_FILES['fileSubImg'], '', $newName, 
			...
			*/					   
			...
			//forward
			$uMesg->showSuccessT('success', $sid, 'sid', $_SERVER['PHP_SELF'], SUPROD201, 'SUCCESS');
...
<table class="tblBrd" align="center" width="650">
	...
	<tr>
	  <td>
	  <form action="<?php echo $_SERVER['PHP_SELF']."?action=Add_Image&sid=".$_GET['sid'];?>"

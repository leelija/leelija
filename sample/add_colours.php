	...
	else
	{
		...
		for($i=0; $i < $selNum; $i++)
			...
			//fileVideo
				...
				$colorId 	= $sample->addColour($sampleDtl[0],$sampleDtl[3],$_POST['txtFabricName'][$i],$_POST['txtColourType'][$i],$_POST['txtColourRatio'][$i],
				$_POST['txtColourRatioValue'][$i],$txtProdDesc,$userData[10]);

				//uploading video
				if($_FILES['uploadvideo']['name'][$i] != '')
				{
					$newName = $utility->getNewName4($i, $_FILES['uploadvideo'], '', $colorId);
					$utility->fileUpload2($i, $_FILES['uploadvideo'], '', $newName, 'video/colour/', 
										$colorId,'upload_video', 'colour_id', 'colour_table');
...
<title><?php echo COMPANY_S; ?> -  - Add Sample Colour</title>
...
<!-- /TinyMCE -->
    ...
    <div class="webform-area">		
		<?php 
        //form
        if( (isset($_GET['action'])) && ($_GET['action'] == 'add_colour') )
        {
             
            $sampleDetails = $sample->showSample($sid);
        ?>
		
<h3><a name="editStock">Add Sample Colours</a></h3>
    
          <form action="<?php $_SERVER['PHP_SELF']?>?action=add_colour&sid=<?php echo $sid; ?>" method="post" enctype="multipart/form-data">
         
		 
		 <h2><a name="addUser">Add Sample Colours</a></h2>
						...
						<!--	<label>Colour Name<span class="orangeLetter">*</span></label>
                            <input name="txtColours" type="text" class="text_box_large" id="txtColours" 
					         size="25" />
                            <div class="cl"></div>-->
							...
							<tr>
									...
									$arr_label	= range(1,8);
									?>
									  <select name="selNum" id="selNum" onchange="return getColourType();"
									   class="textBoxA">

...
if($sampleDtl[0] == '')
	...
	else
	{
				...
				//add the sample manual emb data
				$manual_emb_id 	= $sample->addSamManual($sampleDtl[0],$txtEmpName,$sampleDtl[3],$txtParticularName,$txtTime,
				$txtLabourCost,$labourTotalCost,$MaterialTotalCost,$txtOthersCost,
				$totalCost,$txtProdDesc,$userData[10],'sample_manual_embrotary');
		...
		for($i=0; $i < $selNum; $i++)
			{
				$sampleDtl				= $sample->showSampleDb($sid);
				$sampleManualEmbDtl		= $sample->showSamManualEmb($manual_emb_id);
				
				$materialCost  		= $_POST['txtMatAmount'][$i] * $_POST['txtMaterialRate'][$i];	
				$MTotalCost 		= $sampleManualEmbDtl[10] + $materialCost; // Manual Emb Total cost
				$material_cost 		= $sampleManualEmbDtl[8] + $materialCost;
				
				$total_time 		= $sampleDtl[4];
				$TotalCost 			= $sampleDtl[5] + $materialCost;
				
				//add the sample_manualari_materials 
				$sample->addSamMaterials($manual_emb_id,$_POST['txtMaterialName'][$i],$_POST['txtMatAmount'][$i],
				$_POST['txtMUnit'][$i],$_POST['txtMaterialRate'][$i],$materialCost,'sample_manualemb_materials','manual_emb_id');	
				
				//update sample_manual_aari
				$sample->UpdateSampleAllTable($manual_emb_id,$material_cost,$MTotalCost,'sample_manual_embrotary','manual_emb_id');
				// Update sample_db
...
<title><?php echo COMPANY_S; ?> -Add Sample Manual Embroidery  Material</title>
...
<!-- /TinyMCE -->
    ...
    <div class="webform-area">		
		<?php 
        //form
        if( (isset($_GET['action'])) && ($_GET['action'] == 'add_manual') )
        {
...
<h3><a name="editStock"></a></h3>
          ...
          <form action="<?php $_SERVER['PHP_SELF']?>?action=add_manual&sid=<?php echo $sid; ?>" method="post" enctype="multipart/form-data">
         
		 
		 <h2><a name="addUser">Add Sample Manual Embroidery Material</a></h2>

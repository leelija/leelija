...
else
{
	...
	$sid = $statusCat->getAllFinalStich('added_on','DESC',$userId);
}
/*	GET ALL PRODUCT*/
//print_r($pids);exit;
...
<title><?php echo COMPANY_S; ?> - final stitch Status</title>
    ...
    <div class="container">
        <div class="inner-container">
            ...
            <div id="admin-body">
			  ...
			  <!-- Download option -->
                <div id="options-area">
					<a href="file_download.inc.php?table=final_stich">Download</a>
					<a href="final-stitch-download.php">View & Download</a>
                </div>
                <!-- eof Download option -->
                ...
                <div id="data-column">
                	...
                	<table class="single-column" cellpadding="0" cellspacing="0">
                        ...
                        <thead>
                          ...
                          <th width="18%"  >Particular(complete)</th>
						  <th width="18%"  >Particular</th>
                       ...
                       $sid = array_slice($sid, $start, $limit);     
                        foreach($sid as $x)
							...
							$fStichSampleDtls	= $sample->getAllFinalStichDtlDisplay($statusCatDetail[3]);
							$tpartc 			= count($fStichSampleDtls) * $statusCatDetail[7];
							
							
							$ordDtls 		= $order->showOrders($statusCatDetail[2]);
							// update factory
							$status->updatePipeLine($statusCatDetail[2],$ordDtls[14],'final_stich');
							//echo $tpartc;exit;
							// Update final stitch status table
							
							//$statusCat->updateFSstatus($tpartc, 'Complete');
							
							$bgColor 			= $utility->getRowColor($k);	
                    ?>
                    <tr align="left"<?php $utility->printRowColor($bgColor);?>>
						<td align="center"><a href=""><?php echo $statusCatDetail[2]; ?></a></td>
						<td align="center"><?php echo $statusCatDetail[3]; ?></td>
						<td align="center">
                          <p style='color:blue;' onClick="MM_openBrWindow('status_details.php?action=status_details&oid=<?php echo $statusCatDetail[2]; ?>','AdminDelete','scrollbars=yes,width=500,height=400')">
                          <?php	$order->TotalQuantitySum($statusCatDetail[2]);	?>	 </p> 
						</td>
						<td align="center"><?php echo $customerDetail[5]; ?></td>
						<td align="center">
						  <p onClick="MM_openBrWindow('stitch-part-dtls.php?action=particular-dtls&sid=<?php echo $statusCatDetail[3]; ?>','AdminDelete','scrollbars=yes,width=700,height=600')">
							<?php echo $statusCatDetail[17];echo'<br>'; 
						...
						if($statusCatDetail[7]>0)
						   ...
						   ?>		  
					    <td align="center">
							<p href="#" onClick="MM_openBrWindow('stcolor-change.php?action=bill_details&id=<?php echo $statusCatDetail[0]; ?>','AdminDelete','scrollbars=yes,width=500,height=400')">
								<?php echo $statusCatDetail[18]; ?>
							</p> 
						</td>
						
						<td align="center"><?php echo $statusCatDetail[7]; ?></td>
						<td align="center">
                          <p style='color:blue;' onClick="MM_openBrWindow('final_stich_complete.php?action=final_stichcomp&final_sid=<?php echo $statusCatDetail[0]; ?>','AdminDelete','scrollbars=yes,width=500,height=400')">
						...
						<?php 
						if($statusCatDetail[7] < $statusCatDetail[15])
						{
							$status->updateStableSt($statusCatDetail[2],'Complete','fstiching');
						}
					   if($statusCatDetail[7] <= $statusCatDetail[15])
						{
							$statusCat->editFinalstichstatus($statusCatDetail[0],'Complete');
					   ?>
						<td align="center"><a href="packing_pending.php?ord_id=<?php echo $statusCatDetail[2];?>"><?php echo $statusCatDetail[16]; ?></a></td>
						<?php
						  }
						  else{
								if($statusCatDetail[15]==0)
								{
									...
									{
										$statusCat->editFinalstichstatus($statusCatDetail[0],'Running');
									}	
									else{
										$statusCat->editFinalstichstatus($statusCatDetail[0],'Incomplete');
									}
								}
								else{
									$statusCat->editFinalstichstatus($statusCatDetail[0],'Running');
					   ...
					   <?php
                        ...
                        <td  align="center">
                          <p onClick="MM_openBrWindow('final_stich_update.php?action=update_fstich&final_stich_id=<?php echo $statusCatDetail[0]; ?>','AdminDelete','scrollbars=yes,width=700,height=600')">
                          Update					  </p> 
						  
                          <p onClick="MM_openBrWindow('add_bill.php?action=update_fstich&final_stich_id=<?php echo $statusCatDetail[0]; ?>','AdminDelete','scrollbars=yes,width=700,height=600')">
                          Add Bill					  </p> 

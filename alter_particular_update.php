...
{	
	...
	$alterpending	= $altPartDetail[7] - $txtAmount;// alter quantity calculate
	
	// packing pending quantity calculate
	$alterqnt	= $packingpendDtl[17] - $txtAmount; // $alterqnt	= alter quantity
	$compqnt	= $packingpendDtl[16] + $txtAmount; // $compqnt		= complete quantity
	// total quantity
	$total_particular = $txtAmount + $PackingDtl[18];
	// total labour income
	$workprice = $txtAmount * $alterrateDtl[3];
	
	// Alter Particular table update
	$alterParticular->UpdatealterPerticular($dhmckfip,$alterpending);
	
	// update no_of_particular of packing_table 
	$statuscat->updatePackingQuan($altPartDetail[1],$total_particular);
	
	// update packing pending table
	$pipestatus->EditPackinPend($altPartDetail[8],$compqnt,$alterqnt);
	...
	$labour->editLabour($txtEmpid,$FinalStichDtl[0],$labourDtl[1],$labourDtl[12],$labourDtl[2],$labourDtl[3],$totalearn,$labourDtl[5],$labourDtl[10]);
...
<table class="tblBrd" align="center" width="100%">
	...
	<?php 
    //delete form
    if( (isset($_GET['action'])) && ($_GET['action'] == 'update_altpart') )
    {
    ...
    ?>
    <tr class=''>
	  <td height="25" align='left' bgcolor="#EEEEEE"><h3>Alter  Order id:: <?php echo $altPartDetail[1]; ?></h3></td>
    </tr>
    <tr>
      <td>
      <form action="<?php $_SERVER['PHP_SELF']?>?dhmckfip=<?php echo $dhmckfip; ?>" method="post">
        <table width="100%"  border="0" cellspacing="0" cellpadding="0">
			...
			value="<?php echo $altPartDetail[7];?>" size="25" />
            <div class="cl"></div>
				Rate:<?php echo	$alterrateDtl[3];?>		
			<div >
            ...
            <div class="cl"></div>
				...
				<input style="background:#fc7c03; color: #fff;" name="btnUpdateAlt" type="submit" class="buttonYellow" id="btnDeleteOrd" value="Update" />

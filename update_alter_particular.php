...
{	
	...
	$alterqunt	= $packingpendrecord[17] + $txtAmount; // $alterqunt	= alter quantity
	
	$total_particular = $statusCatDetail[7] + $PackingDtl[18];
	if($txtAmount	== ""){
		echo "Put the Quantity values";
	}
	elseif($txtLabour	== ''){
		echo "Put the Labour values";
	}
	elseif($txtReason	== ''){
		echo "Put the Reason";
	}
	elseif($statusCatDetail[7] >= $txtAmount){
	
		$alterParticular->addAlterpart($packingpendrecord[0],$statusCatDetail[2],$statusCatDetail[3],$statusCatDetail[5],$statusCatDetail[13],$txtAmount,$txtLabour,$txtReason);
		
		// update packing pending table
		$pipestatus->Updatependintblbyalter($statusCatDetail[2],$statusCatDetail[5],$statusCatDetail[13],$pendingqnt,$alterqunt);
		//delete the stock
...
<table class="tblBrd" align="center" width="100%">
    ...
    ?>
    <tr class=''>
	  <td height="25" align='left' bgcolor="#EEEEEE"><h3> Packing Pending Order id:: <?php echo $statusCatDetail[2]; ?></h3></td>
    </tr>
    <tr>
      <td>
      <form action="<?php $_SERVER['PHP_SELF']?>?dhmckfip=<?php echo $dhmckfip; ?>" method="post">
        <table width="100%"  border="0" cellspacing="0" cellpadding="0">
			...
			<label>Alter(in pieces)<span class="orangeLetter">*</span></label>
			...
			<label>Reason<span class="orangeLetter">*</span></label>
            <input name="txtReason" type="text" class="text_box_large" id="txtReason" 
			value="" size="25" />
            <div class="cl"></div>

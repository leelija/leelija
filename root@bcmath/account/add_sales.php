...
if(isset($_POST['Submit'])) { //check if form submitted
	...
	$txtBillingOn		= $_POST['txtBillingOn'];
	$tax				= $_POST['tax'];
	$tax_dis			= $_POST['tax_dis'];
	$description		= $_POST['description'];
	
	
	$cgst_amount		= $payable * ($cgst / 100);	
	$sgst_amount		= $payable * ($sgst / 100);	
	$igst_amount		= $payable * ($igst / 100);
	$txtNetBalance		= $payable + $cgst_amount + $sgst_amount + $igst_amount;
	
	//registering the post session variables
	$sess_arr	= array('supplier','bill_no', 'BillingOn', 'subtotal', 'discount',
		'dis_amount','sgst','cgst_amount','cgst');
	...
	if($supplier=='')
	{
		$msg="Billing name field empty" ;
	}
	
	else
	{
		//Add New Invoice
		$invoice_id = $invoice->addInvoice($supplier,$bill_no,$txtBillingOn, $subtotal,$discount,$dis_amount,$sgst,$cgst_amount, 
				$cgst,$sgst_amount,$igst,$igst_amount,$tax,$tax_dis,$txtNetBalance,$description,$billDate,$userData[2]);
				
		$custDtls 		= $buyer->showBuyerDtlsComp($supplier);
		$cBalance 		= $custDtls[12] + $txtNetBalance;
		
		//Edit Customer Record
		$buyer->editBuyer($custDtls[0],$custDtls[1], $custDtls[14],$custDtls[2], $custDtls[4], $custDtls[15], $cBalance, $userData[2]);
		
		//Account details
		$cAccDtls 		= $company->showCompDtls($txtBillingOn);
		$otsgst			= $cAccDtls[8] + $sgst_amount;
		$otcgst			= $cAccDtls[9] + $cgst_amount;
		$otigst			= $cAccDtls[10] + $igst_amount;
		//update company
		$company->updateCompDtls($cAccDtls[0],$cAccDtls[1],$cAccDtls[2], $cAccDtls[17],$cAccDtls[3],$cAccDtls[4],$cAccDtls[5], $cAccDtls[6], 
		$cAccDtls[7], $otsgst,$otcgst, $otigst,$userData[2]);
		
		//buyer details
		$buyerDtls 		= $buyer->showBuyerAccCompWise($custDtls[0],$txtBillingOn);
		if($custDtls[0] == $buyerDtls[1] && $txtBillingOn == $buyerDtls[2])
			{
				$ctBalance 	= $buyerDtls[3] + $txtNetBalance;
				$buyer->UpdateBuyerAccBlnc($custDtls[0],$txtBillingOn, $ctBalance);
			}
		else{
				$buyer->addBuyerAcc($custDtls[0],$txtBillingOn, $txtNetBalance);
			}
		//invoice details
		for($i=0; $i < count($_POST['quty']); $i++)
			{
						/*$quantity=$_POST['quty'][$i];
						$rate=$_POST['sell'][$i];
						$total=$_POST['total'][$i];*/
				//Add Invoice Details
				$invoice->addInvoiceDtls($invoice_id, $_POST['stock_name'][$i], $_POST['quty'][$i], $_POST['sell'][$i], $_POST['total'][$i]);
				//echo $_POST['item'][$i];exit;
			}
		
		//deleting the sessions
		$utility->delSessArr($sess_arr);
			
		$msg="Sales Added successfully Ref: ". $bill_no."" ;
...
$(function() {
    	...
    	$("#supplier").autocomplete("customer.inc.php", {
		width: 160,
		autoFill: true,
		selectFirst: true
	});
        ...
        $("#supplier").blur(function()
			{
			 ...
			 $.post('customer-dtls.inc.php', {stock_name1: $(this).val() },
				function(data){
		...
		<div class="page-full-width cf">
			...
			<div class="side-menu fl">
				...
				<ul>
					...
					<li><a href="view-selling.php">View Sales</a></li>
				</ul>
			</div> <!-- end side-menu -->
			...
			<div class="side-content fr">
				<div class="content-module">
					...
					?>
						...
						<form name="form1" method="post" id="form1" action=""><!--start form-->
							...
							<div class="mytable_row "><br>
								<table class="form "  border="0" cellspacing="0" cellpadding="0">
									...
									<tr>  <td>&nbsp; </td> <td>&nbsp; </td>
										<td><span class="man">*</span>Billing Name:</td>
										<td>
							...
							<div align="center">
								...
								<table class="form" >
									...
									<tr>
										<td>
											<input name="item"  type="text" id="item"  maxlength="200"  class="round default-width-input " style="width: 150px"   />
										</td>
						...
						<div class="mytable_row ">
							<table class="form">
								...
								<tr> 
									<td>&nbsp; </td>
									<td style="display: none;" >Payment:<input type="text"  class="round" onkeyup=" balance_amount(); " onkeypress="return numbersonly(event);" name="payment" id="payment" >
									</td>
									<td style="display: none;" >Balance:<input type="text"  class="round" readonly id="balance" name="balance" >               
									</td>
									<tr></tr>
									<td>&nbsp; </td>
									<td>Without GST Amount:<input type="hidden" readonly id="grand_total"  > 
								...
								<tr> 
									<td>&nbsp; </td>
									<td>CGST<input type="text" maxlength="3" class="round" onkeyup=" CGST_amount(); " onkeypress="return numbersonly(event);" value="0" name="cgst" id="cgst">
									</td>
								...
								<tr> 
									<td>&nbsp; </td>
									<td>SGST<input type="text" maxlength="3" class="round" onkeyup=" sgst_amount(); " onkeypress="return numbersonly(event);" value="0" name="sgst" id="sgst" >
									</td>
									<td>SGST Amount:<input type="text" disabled onkeypress="return numbersonly(event);"  onkeyup=" sgst_as_amount(); "  class="round" id="sgst_amount" name="sgst_amount" >               
									</td>
									
								</tr>
								<tr> 
									<td>&nbsp; </td>
									<td>IGST<input type="text" maxlength="3" class="round" onkeyup=" igst_amount(); " onkeypress="return numbersonly(event);" value="0" name="igst" id="igst" >
									</td>
									<td>SGST Amount:<input type="text" disabled onkeypress="return numbersonly(event);"  onkeyup=" sgst_as_amount(); "  class="round" id="igst_amount" name="igst_amount" >               
									</td>
								...
								<tr> 
									<td>&nbsp; </td>
									<td>Net Billing Amount<input type="text" disabled maxlength="3" class="round" onkeypress="return numbersonly(event);" name="txtNetBalance" id="txtNetBalance" >
									</td>
								...
								<tr> 
									<td>Billing On &nbsp;</td>
									<td>
									<select name="txtBillingOn" type="text" id="txtBillingOn" class="text_box_large">
									<?php
...
$(document).ready(function() {
	...
	//alert("hi..");
		$("input[name='payable']").each(function (index) {
			var payable  		= $("input[name='payable']").eq(index).val();
            var cgst  			= $("input[name='cgst']").eq(index).val();
			var cgst_amount		= parseInt(payable) * parseInt(cgst) / 100;
	...
	//alert("hi..");
       // var grandTotal = 0;
        $("input[name='payable']").each(function (index) {
            var payable  		= $("input[name='payable']").eq(index).val();
            var cgst 			= $("input[name='cgst']").eq(index).val();
			var sgst 			= $("input[name='sgst']").eq(index).val();
			var sgstr			= parseInt(payable) * parseInt(sgst) / 100;
			var cgstr			= parseInt(payable) * parseInt(cgst) / 100;
            var txtNetBalance 	= parseInt(payable) + parseInt(sgstr) + parseInt(cgstr);
			//alert(txtNetBalance);
			$("#txtNetBalance").val(txtNetBalance);
			$("#sgst_amount").val(cgstr);
		});
    });
	...
	//alert("hi..");
       // var grandTotal = 0;
        $("input[name='payable']").each(function (index) {
            ...
            var cgst 			= $("input[name='cgst']").eq(index).val();
			var sgst 			= $("input[name='sgst']").eq(index).val();
			var igst 			= $("input[name='igst']").eq(index).val();
			var sgstr			= parseInt(payable) * parseInt(sgst) / 100;
			var cgstr			= parseInt(payable) * parseInt(cgst) / 100;
			var igstr			= parseInt(payable) * parseInt(igst) / 100;
            var txtNetBalance 	= parseInt(payable) + parseInt(sgstr) + parseInt(cgstr)+ parseInt(igstr);
			//alert(txtNetBalance);
			$("#txtNetBalance").val(txtNetBalance);
			$("#igst_amount").val(igstr);

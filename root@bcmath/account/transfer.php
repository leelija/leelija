...
$ca_id			= $utility->returnGetVar('ca_id','0');

//User detail
$userData 		=  $customer->getCustDatabyEmail($_SESSION[ACC_SESS]);

$bAccDtls		=  $company->showCompAcc($ca_id);
...
{	
	...
	$toAccDtls		=  $company->showCompAcc($txtTranTo);
	...
	else
	{
		...
		$currtoBalance 	= $toAccDtls[4] + $txtBalance;
		
		//update company account
		$company->updateCompAcc($ca_id,$bAccDtls[1],$bAccDtls[2], $bAccDtls[3],$currBalance,$bAccDtls[5], $bAccDtls[6], 
		$bAccDtls[7], $bAccDtls[8],$bAccDtls[9], $bAccDtls[10],$userData[2]);
		//update to company account
		$company->updateCompAcc($txtTranTo,$toAccDtls[1],$toAccDtls[2], $toAccDtls[3],$currtoBalance,$toAccDtls[5], $toAccDtls[6], 
		$toAccDtls[7], $toAccDtls[8],$toAccDtls[9], $toAccDtls[10],$userData[2]);
		
		//add the Company Transaction Records
		$company->addCompTran($txtTranType, $txtPayType, $txtCheqeNo,$ca_id,$toAccDtls[1],$txtBalance,'dr',
		$currBalance,$txtProdDesc,$userData[2]);
		
		$company->addCompTran($txtTranType, $txtPayType, $txtCheqeNo,$txtTranTo,$bAccDtls[1],$txtBalance,'cr',
		$currtoBalance,$txtProdDesc,$userData[2]);
		
		//deleting the sessions
		$utility->delSessArr($sess_arr);
		
		//forward the web page
		$uMesg->showSuccessT('success', 0, '', 'company-account.php', "Transaction has been successfully completed", 'SUCCESS');
	}
            ...
            <div id="admin-body">
                ...
                <div class="webform-area">
					...
					?>
                        ...
                        <form action="<?php $_SERVER['PHP_SELF']?>?action=create_Tran&ca_id=<?php echo $ca_id; ?>" method="post" enctype="multipart/form-data">
                            <div class="cl"></div>
							<h2>Transfer:</h2>
							<div >
								<label> From<span class="orangeLetter">*</span></label>							
								<select name="txtTranFrom" type="text" id="txtTranFrom" class="text_box_large">
									<option value="<?php echo $bAccDtls[0];?>"><?php echo $bAccDtls[1];?>(<?php echo $bAccDtls[2];?>)</option> 
								</select>
							</div> 
                            <div class="cl"></div>
							<div >
								<label>To<span class="orangeLetter">*</span></label>							
								...
								?>
									<option value="<?php echo $row['ca_id'];?>"><?php echo $row['account_name'];?>(<?php echo $row['bank'];?>)</option> 
								<?php

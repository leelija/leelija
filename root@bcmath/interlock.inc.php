...
if( (isset($_GET['selNum'])) && ($_GET['selNum'] > 0)  && ($_GET['selNum'] < 70))
{
	$selNum		= $_GET['selNum'];
	...
	{
	?>  
    	<h3>Interlock record-<?php echo $i; ?></h3>
        <div class="cl"></div>
		...
		<div >
				<label>Interlock For<span class="orangeLetter">*</span></label>							
				<select name="txtInterlockRateId[]" type="text" id="txtInterlockRateId" class="text_box_large">
					<?php
						$interlockDetails         = $rate->getInterlockRateDisp();
						foreach ($interlockDetails as $row){//Array or records stored in $row
						//echo $row['kali_type'];exit;
						?>
						<option value=<?php echo $row['interlock_rate_id'];?>><?php echo $row['particular'];?></option>
					<?php
		...
		<label>Interlock Quantity</label>

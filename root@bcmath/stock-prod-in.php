...
//	$txtColour			= $_POST['txtColour'];//colour
	...
	for($i=0; $i < $selNum; $i++)
		...
		$ordDtlsCnt 	= $orders->getOrderQuantitycount($packingDetail[2]);
		foreach($ordDtlsCnt as $eachQty)
			{
				$OrdQty 	= $eachQty['SUM(quantity)']; 
			}
		if($OrdQty  == $completeTotal)
			{
				$txtStatus				= 'complete';
			}
		else{
		...
		{
			echo "Please Put the correct value";exit;
		}
...
<script type="text/javascript" src="js/stock-prod-in.js"></script>
<!-- eof JS Libraries -->
...
<!-- /TinyMCE -->
    ...
    <div class="webform-area">		
		<?php 
        //form
        if( (isset($_GET['action'])) && ($_GET['action'] == 'ProdIn') )
        {
        ...
        <form action="<?php $_SERVER['PHP_SELF']?>?action=ProdIn&sid=<?php echo $sid; ?>" method="post" enctype="multipart/form-data">
            ...
            <input name="txtDesignNo" type="text" class="text_box_large" id="txtDesignNo" value="<?php echo $packingDetail[3] ?>" size="25" />
            <div class="cl"></div>
			...
			<tr>
				...
				?>
					<select name="selNum" id="selNum" onchange="return getAllSProdsIn(<?php echo $sid; ?>);" class="textBoxA">

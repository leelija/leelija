...
{	
	...
	$txtBillNo 	            = '';		//Bill no
	$txtBillingName 	    = $_POST['txtBillingName']; // Fabric id
	...
	else
	{
			...
			if($ordDtls[14] == 2)
				{
					...
					if($dyeCat == "Dyeable")
						...
						$txtBillNo 	 = $bill->addBill($txtEmpId,'','','',$txtBillingName,$attDate, $userData[2],'No','rfri_dye_bbook');
						for($i=0; $i < $selNum; $i++)
						{
							...
							$TargetDate,0,'Running',$userData[2],$_POST['txtNoOfColor'][$i],$_POST['txtFabricAmount'][$i],$dyeCat,$ordDtls[14]);	
						...
						for($i=0; $i < $selNum; $i++)
						{
								...
								$TargetDate,$tFabAmount,'complete',$userData[2],$_POST['txtNoOfColor'][$i],$_POST['txtFabricAmount'][$i],$dyeCat,$ordDtls[14]);	
								// $fabric->editFabric($fid,$fabricDtl[1],$current_stock,$fabricDtl[5],$fabricDtl[4]);
				...
				$dyeCat 	= $_POST['txtFabricCat'];
					if($dyeCat == "Dyeable")
						{
						//add bill in the Dye bill table
						$txtBillNo 	 	= $bill->addBill($txtEmpId,'','','',$txtBillingName,$attDate, $userData[2],'No','mep_dye_bbook');
						//$billNo			= D'.'$txtBillNo;
						$billNo			= "D".$txtBillNo."";
						for($i=0; $i < $selNum; $i++)
						{
			...
			if($ordDtls[14] == 2)//Bill for HMDA
				{
					...
					$txtBillNo 	 = $bill->addBill($txtEmpId,'','','',$txtBillingName,$attDate, $userData[2],'No','rfri_hand_bbook');
				
				if($txtWorkType == "PureHand" OR $txtWorkType == "AllWork")	
					{
						...
						// Cost Calculate		
						$sHandDtls		= $sample->getAllHandDtlsPartWise($prodstatDtl[2],$txtWorkType,$_POST['txtParticular'][$i]);
						
						$tAmount		= $sHandDtls[5] * $sHandDtls[6] + $sHandDtls[8]+ $sHandDtls[9];
						...
						if($dueQty >= 0)
							...
							'','','','',$txtProdDesc,$OrderDate,$TargetDate,$txtWorkType,$_POST['txtParticular'][$i]);
					...
					*/
						...
						// Cost Calculate		
						$sHandDtls		= $sample->getAllHandDtlsPartWise($prodstatDtl[2],'PureHand',$_POST['txtParticular'][$i]);
					...
					// Add Hand bill 
					//$txtBillNo 	= $bill->addBill($txtEmpId,$attDate, $userData[2],'No','rfri_hand_bbook');
					$txtBillNo 	 = $bill->addBill($txtEmpId,'','','',$txtBillingName,$attDate, $userData[2],'No','mep_hand_bbook');
					$billNo			= "H".$txtBillNo."";
				...
				if($txtWorkType == "PureHand" OR $txtWorkType == "AllWork")	
					{
						...
						*/	
						...
						$sHandDtls		= $sample->getAllHandDtlsPartWise($prodstatDtl[2],'HighLight',$_POST['txtParticular'][$i]);
						$tAmount		= $sHandDtls[5] * $sHandDtls[6] + $sHandDtls[8]+ $sHandDtls[9];
		...
		elseif($txtStatus=='Manual')
		{
			...
			if($ordDtls[14] == 2)
				...
				$txtBillNo 	 = $bill->addBill($txtEmpId,'','','',$txtBillingName,$attDate, $userData[2],'No','rfri_manual_bbook');
				...
				for($i=0; $i < $selNum; $i++)
					...
					$txtBillNo 	 	= $bill->addBill($txtEmpId,'','','',$txtBillingName,$attDate, $userData[2],'No','mep_manual_bbook');
					$billNo			= "M".$txtBillNo."";
		...
		{
			$billName 	= 'Computer';
			$txtMarea 	= $_POST['txtMarea'];
			
			if($ordDtls[14] == 2)
				{
					// Add Computer bill 
					//$txtBillNo 	= $bill->addBill($txtEmpId,$attDate, $userData[2],'No','rfri_comp_bbook');
					$txtBillNo 	 = $bill->addBill($txtEmpId,'','','',$txtBillingName,$attDate, $userData[2],'No','rfri_comp_bbook');
					...
					for($i=0; $i < $selNum; $i++)
					{
						...
						$_POST['txtStitchRate'][$i],'','','',$_POST['txtWorkType'],$txtMarea,$txtProdDesc,$OrderDate,$TargetDate,$userData[2],$_POST['txtNoOfHead'][$i]
						,$_POST['txtNoOfColor'][$i],$_POST['txtUnit'][$i],$_POST['txtAdditional'][$i]);
						
						//computer billing update
						$compBillDtls 	= $bill->allRFRIBillTable($txtBillNo,'rfri_comp_bbook');
						$partQuantity 	= $compBillDtls[7] + $Quantity;
						$gCost 			= $Quantity * $_POST['txtStitchRate'][$i];
						$labCost 		= $compBillDtls[9] + $gCost;
						//update bill table
						$bill->UpdateAllBill($txtBillNo,$partQuantity,'',$labCost,'rfri_comp_bbook');
						
					}
						//Update Plan
						$plan->updatePlanforComputer($prodstatDtl[1],$OrderDate);
						if($_POST['txtWorkType'] == "Embroidery" OR $_POST['txtWorkType'] == "Multi")
							{
								//Update status table		
								$prodStatus->editStatus($psid,$prodstatDtl[1],$prodstatDtl[2], $prodstatDtl[19],$prodstatDtl[3],
								...
								//Update status table		
								$prodStatus->updateAriStat($psid,'Running');
							}
				}
				else{
					...
					$txtBillNo 	 = $bill->addBill($txtEmpId,'','','',$txtBillingName,$attDate, $userData[2],'No','mep_comp_bbook');
					$billNo			= "C".$txtBillNo."";
					//$txtColor 	= $_POST['txtColor'];
					for($i=0; $i < $selNum; $i++)
					{
						...
						//computer billing update
						$compBillDtls 	= $bill->allRFRIBillTable($txtBillNo,'mep_comp_bbook');
						$partQuantity 	= $compBillDtls[7] + $Quantity;
						$gCost 			= $Quantity * $_POST['txtStitchRate'][$i];
						$labCost 		= $compBillDtls[9] + $gCost;
						//update bill table
						$bill->UpdateAllBill($txtBillNo,$partQuantity,'',$labCost,'mep_comp_bbook');
			...
			if($ordDtls[14] == 2)//for HMDA
				{
					...
					$txtBillNo 	 = $bill->addBill($txtEmpId,'0.0','0.0','0.0',$txtBillingName,$attDate, $userData[2],'No','rfri_fs_bbook');
					for($i=0; $i < $selNum; $i++)
					{
						...
						//	{
								...
								$_POST['txtColor'][$i],$_POST['txtFabricAmount'][$i],$_POST['txtParticular'][$i],'','','',$txtProdDesc,$OrderDate,$TargetDate);
						/*	}
						else{
							echo "Bill already Generate";exit;
						}*/
					}
					...
					// Add Final Stiching bill 
					//$txtBillNo 	= $bill->addBill($txtEmpId,$attDate, $userData[2],'No','rfri_fs_bbook');
					$txtBillNo 	 	= $bill->addBill($txtEmpId,'0.0','0.0','0.0',$txtBillingName,$attDate, $userData[2],'No','mep_fs_bbook');
					$billNo			= "F".$txtBillNo."";
					for($i=0; $i < $selNum; $i++)
					{
						//echo $_POST['txtColor'][$i];
...
document.getElementById("msg").style.display='inline';
    ...
    <div class="webform-area">		
        ...
        ?>
          ...
          <form action="<?php $_SERVER['PHP_SELF']?>?action=update_status&psid=<?php echo $psid; ?>" method="post" enctype="multipart/form-data" id="sbmtForm">
		 ...
		 <h2>->Order Id:<?php echo $statusDetails[1];?>&nbsp;->Design No:<?php echo $statusDetails[2];?><br><br>
                        ...
                        <!--<form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data">-->
                            <div class="cl"></div>
							<div >
								<label>Billing For<span class="orangeLetter">*</span></label>							
									<select name="txtBillingName" type="text" id="txtBillingName" class="text_box_large">
										<?php
											$FactDetails         = $sample->getAllFactory();
											foreach ($FactDetails as $row){//Array or records stored in $row
											//echo $row[colour_name];
										?>	
											<option value="<?php echo $row['factory_id'];?>"><?php echo $row['factory_name'];?></option>
...
<?php /*?>                            <label>Tax</label>
                ...
                <input id="btnSubmit" name="btnEditOrd" type="submit" class="button-add"  value="asign" />

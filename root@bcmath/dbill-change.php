...
//Dyeing Details
$DyeingDtl		= $statuscat->showProductStatCat($id);
// echo $DyeingDtl[1];exit;

$ordDtls 		= $orders->showOrders($DyeingDtl[2]);
...
if($txtEmpId == '')
	{
		echo "Dyer Name empty";
	}
	else
	{
		// update billing name from dyeing table
	   $statuscat->UpdatePBillingName($DyeingDtl[17], $txtEmpId,'dyeing_table');
	   
	   if($ordDtls[14] == 2){
				// Update Billing Name
				$bill->UpdateBillName($DyeingDtl[17], $txtEmpId,'rfri_dye_bbook');
			}
		else{
				$str = $DyeingDtl[17];
				...
				$billNo = $arr[1];
				// Update Billing Name
				$bill->UpdateBillName($billNo, $txtEmpId,'mep_dye_bbook');
		...
		$uMesg->showSuccessT('success', 0, '', $_SERVER['PHP_SELF'], 'Dyeing billing name has been changed', 'SUCCESS');
	}
}
...
<title><?php echo COMPANY_S; ?> -  - EDIT Dyeing Bill</title>
...
<!-- /TinyMCE -->
    ...
    <div class="webform-area">		
        ...
        {
            $DyeingDtl		= $statuscat->showProductStatCat($id);
			$custDtls		= $customer->getCustomerData($DyeingDtl[5]);
			
        ?>
		
        <form action="<?php $_SERVER['PHP_SELF']?>?action=bill_details&id=<?php echo $id; ?>" method="post" enctype="multipart/form-data">
			<h2><a name="addUser">Dyeing Bill Changed</a></h2>
			<h2>->Order Id <?php echo $DyeingDtl[2];?>&nbsp;->Design No.<?php echo $DyeingDtl[3];?>&nbsp;->Bill No. D<?php echo $DyeingDtl[17];?></h2>
			
			<div >
				<label>Employee Name<span class="orangeLetter">*</span></label>							
				<select name="txtEmpId" type="text" id="txtEmpId" class="text_box_large">
				<option value="<?php echo $DyeingDtl[5];?>"><?php echo $custDtls[2];?></option>
				<?php

...
{	
	...
	$leftAmount			= $ComputerDtl[7]-$txtAmount;
	$completeTotal		= $ComputerDtl[15]+$txtAmount;	
...
if($txtAmount == '')
	...
	{
		if($txtAmount <= $ComputerDtl[7]){
		//Edit Computer table
	   $statuscat->editComputer($dhmckfip,$ComputerDtl[1],$ComputerDtl[2],$ComputerDtl[3],$ComputerDtl[4],$ComputerDtl[5],$ComputerDtl[6],
	   $leftAmount,$ComputerDtl[8],$ComputerDtl[9],$ComputerDtl[10],$customerDtl[16],$completeTotal,$txtStatus,
	   $ComputerDtl[17],$ComputerDtl[18],$ComputerDtl[19]);	
	   
	   $prodStatusDtl	=$prodStatus->showProductStat($ComputerDtl[1]);
	   
		if($ComputerDtl[20] == "Embroidery" OR $ComputerDtl[20] == "Multi")
			{
				//Update status table		
				$prodStatus->editStatus($ComputerDtl[1],$prodStatusDtl[1],$prodStatusDtl[2], $prodStatusDtl[19],$prodStatusDtl[3],
				$prodStatusDtl[4], $prodStatusDtl[5],$prodStatusDtl[6],$prodStatusDtl[7],$prodStatusDtl[8],$txtStatus,$prodStatusDtl[9],
				$prodStatusDtl[10],$prodStatusDtl[11],$prodStatusDtl[12],$prodStatusDtl[13],$prodStatusDtl[20]);
			}
		else{
				//Update status table		
				$prodStatus->updateAriStat($ComputerDtl[1],$txtStatus);
			}
		}
	  else{
			echo "Please check update value";exit;
		 }
...
<title><?php echo COMPANY_S; ?> -  - EDIT Computer status</title>
...
<!-- /TinyMCE -->
    ...
    <div class="webform-area">		
        ...
        if( (isset($_GET['action'])) && ($_GET['action'] == 'update_comp') )
        {
            ...
            $ComputerDetails = $statuscat->showProductComputer($dhmckfip);
        ?>
		
<h3><a name="editStock">Edit Computer status</a></h3>
    
          <form action="<?php $_SERVER['PHP_SELF']?>?action=update_comp&dhmckfip=<?php echo $dhmckfip; ?>" method="post" enctype="multipart/form-data">
         
		 
		 <h2><a name="addUser">Edit Computer status</a></h2>
					 <h2>->Order Id <?php echo $ComputerDetails[2];?>&nbsp;->Design No. <?php echo $ComputerDetails[3];?></h2>
                        <span>Please  <span class="required"></span>fill the amount field.</span>
                        <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data">
                            
                            <div class="cl"></div>
							<label>Computer Amount(in meter)<span class="orangeLetter">*</span></label>
                            <input name="txtAmount" type="text" class="text_box_large" id="txtAmount" 
					        value=<?php echo $ComputerDetails[7];?> size="25" />
                            <div class="cl"></div>
							<div >

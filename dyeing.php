...
//echo $keyword;exit;
	if($_POST['type'] ==""){
		$link = '';
		$sid = $search_obj->getProductKeywordstore($keyword,$userId);
		//echo $sid;exit;
	}
	else{
		$link = '';
		$sid = $advSearch->getCommonSearch($keyword,$type,$userId,'dyeing_id','dyeing_table');
...
else
{
	...
	$sid = $statusCat->getAllDyeing('added_on','DESC',$userId);
}
/*	GET ALL PRODUCT*/
//print_r($pids);exit;
    ...
    <div class="container">
        <div class="inner-container">
            ...
            <div id="admin-body">
			  ...
			  <!-- Download option -->
                <div id="options-area">
					<a href="file_download.inc.php?table=dyeing_table">Download</a>
					<a href="dyeing-download.php">View & Download</a>
                </div>
                <!-- eof Download option -->
                ...
                <div id="data-column">
                	...
                	<table class="single-column" cellpadding="0" cellspacing="0">
                        ...
                        <thead>
						  ...
						  <th width="18%"  >left(meter)</th>
                          <th width="18%"  >complete(meter)</th>
						  <th width="18%"  >Remarks</th>
						  <th width="18%"  >Status</th>
						  <th width="18%"  >Fab.Cat</th>
                       ...
                       $sid = array_slice($sid, $start, $limit);     
                        foreach($sid as $x)
							...
							$statusCatDetail = $statusCat->showProductStatCat($x);
							$customerDetail  = $client->getCustomerData($statusCatDetail[5]);
							
							$ordDtls 		= $order->showOrders($statusCatDetail[2]);
							// update factory
							$prodStatus->updatePipeLine($statusCatDetail[2],$ordDtls[14],'dyeing_table');
                      ...
                      <tr align="left"<?php $utility->printRowColor($bgColor);?>>
					    ...
					    <td align="center">
							<a href="#" onClick="MM_openBrWindow('emp_details.php?action=emp_details&emp_id=<?php echo $statusCatDetail[5]; ?>','AdminDelete','scrollbars=yes,width=500,height=400')">
							 <?php echo $customerDetail[5]; ?>	 
							</a> 
						</td>
						
						<td align="center">
							<p href="#" onClick="MM_openBrWindow('dbill-change.php?action=bill_details&id=<?php echo $statusCatDetail[0]; ?>','AdminDelete','scrollbars=yes,width=500,height=400')">
					   ...
					   <?php
						...
						if($statusCatDetail[7]>0)
						   ...
						   ?>
							<td align="center" style="color:green;"><?php echo $diffordtarget->format("%R%a days"); ?></td>
						   <?php
						   }
						   ?>
					   ...
					   if($statusCatDetail[7] == 0)
						{
							$statusCat->editDyeingstatus($statusCatDetail[0],'Complete');
					   ?>
					   ...
					   <?php
								...
								if($statusCatDetail[15]==0)
								{
									...
									{
										$statusCat->editDyeingstatus($statusCatDetail[0],'Running');
									}	
									else{
										$statusCat->editDyeingstatus($statusCatDetail[0],'Incomplete');
									}
								}
								else{
									$statusCat->editDyeingstatus($statusCatDetail[0],'Running');
					   ...
					   <?php
						...
						<td align="center"><?php echo $statusCatDetail[22]; ?></td>
                         ...
                         [ 
                          <a href="#" onClick="MM_openBrWindow('dyeing_update.php?action=update_dyeing&dhmckfip=<?php echo $statusCatDetail[0]; ?>',align='middle','AdminDelete','scrollbars=yes,width=400,height=300')">
                          Update Dyeing					  </a> ]

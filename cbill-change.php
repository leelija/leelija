...
//Computer Details
$ComputerDtl	= $statuscat->showProductComputer($id);

$ordDtls 		= $orders->showOrders($ComputerDtl[2]);
...
if($txtEmpId == '')
	{
		echo "Computer Operator Name empty";
	}
	
	else
	{
		// update billing name from hand table
	   $statuscat->UpdatePBillingName($ComputerDtl[17], $txtEmpId,'computer_table');
	   
		if($ordDtls[14] == 2){
				// Update Billing Name
				$bill->UpdateBillName($ComputerDtl[17], $txtEmpId,'rfri_comp_bbook');
			}
		else{
				$str = $ComputerDtl[17];
				...
				$billNo = $arr[1];
				// Update Billing Name
				$bill->UpdateBillName($billNo, $txtEmpId,'mep_comp_bbook');
		...
		$uMesg->showSuccessT('success', 0, '', $_SERVER['PHP_SELF'], 'Computer billing name has been changed', 'SUCCESS');
	}
}
...
<title><?php echo COMPANY_S; ?> -  - EDIT Computer Billing Name</title>
...
<!-- /TinyMCE -->
    ...
    <div class="webform-area">		
        ...
        {
            $ComputerDtl	= $statuscat->showProductComputer($id);
			$custDtls		= $customer->getCustomerData($ComputerDtl[5]);
			
        ?>
		
        <form action="<?php $_SERVER['PHP_SELF']?>?action=bill_details&id=<?php echo $id; ?>" method="post" enctype="multipart/form-data">
			<h2><a name="addUser">Computer Billing Name Changed</a></h2>
			<h2>->Order Id <?php echo $ComputerDtl[2];?>&nbsp;->Design No.<?php echo $ComputerDtl[3];?>&nbsp;->Bill No. C<?php echo $ComputerDtl[17];?></h2>
			
			<div >
				<label>Employee Name<span class="orangeLetter">*</span></label>							
				<select name="txtEmpId" type="text" id="txtEmpId" class="text_box_large">
				<option value="<?php echo $ComputerDtl[5];?>"><?php echo $custDtls[5];?></option>
				<?php

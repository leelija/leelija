...
{	
	...
	//add the Sample Fabric bill
	  $bill_id =  $bill->addOtherBilling($txtDesigner, $txtDyerName,$txtPurpose, '', '',$OrdersDate,$TargetDate,$txtProdDesc,$userData[2]);	
		
		for($i=0; $i < $selNum; $i++)
			{
				$tAmount 	= $_POST['txtRate'][$i] * $_POST['txtQuantity'][$i];
				//add the bill details
				$bill->addOtherBillingDtls($bill_id,$_POST['txtFabricName'][$i],
				$_POST['txtQuantity'][$i],$_POST['txtRate'][$i],$tAmount);	
				
				
				$otherBillDtls 	= $bill->allOtherBilling($bill_id);
				$TotalAmount 	= $otherBillDtls[4] + $tAmount;
				$bill->updateOtherBilling($bill_id,$TotalAmount);
			}
		...
		$uMesg->showSuccessT('success', 0, '', 'mep-others-bill.php', "Bill has been successfully added", 'SUCCESS');
	}
...
if(isset($_POST['btnCancel']))
{
	...
	//forward
	header("Location: mep-others-bill.php");
}
?>
...
<title><?php echo COMPANY_S; ?> - Others Billing Details</title>
...
<script type="text/javascript" src="js/sample_billing.js"></script>
                ...
                <div class="webform-area">
					...
					?>
                        ...
                        <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data">
                            ...
                            <label>Bill By<span class="orangeLetter">*</span></label>
							...
							<label>Purpose</label>
                            <input name="txtPurpose" type="text" class="text_box_large" id="txtPurpose" 
					        value="<?php $utility->printSess2('txtPurpose',''); ?>" size="25" />
                            <div class="cl"></div>
							...
							<tr>
								<label>Select No. of Particulars</label>
									...
									<?php 
									//gen number array
									$arr_value	= range(1,50);
									$arr_label	= range(1,50);
									?>
									  <select name="selNum" id="selNum" onchange="return getSampBilling();"
									   class="textBoxA">
